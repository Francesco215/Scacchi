...
def test_search_counters_are_exact_and_do_not_perturb_seeded_outputs():
    root = dirichlet_mctx.RootFnOutput(
        prior_logits=jnp.zeros((2, 2), dtype=jnp.float32),
        value=jnp.ones((2, 2), dtype=jnp.float32),
        action_values=jnp.ones((2, 2, 2), dtype=jnp.float32),
    ...
    def recurrent_fn(_, rng_key, action, embedding):
        ...
        return (
            dirichlet_mctx.RecurrentFnOutput(
                value=jnp.ones((2, 2), dtype=jnp.float32),
                action_values=jnp.ones((2, 2, 2), dtype=jnp.float32),
                invalid_actions=jnp.zeros((2, 2), dtype=jnp.bool_),
    ...
    assert jnp.array_equal(first.action, second.action)
    assert jnp.array_equal(first.action_weights, second.action_weights)
    assert jnp.array_equal(
        first.search_tree.summary().alpha,
        second.search_tree.summary().alpha,
    )
    assert jnp.array_equal(
        first.search_tree.simulation_active_count,
        jnp.asarray([3, 0], dtype=jnp.int32),
    )
    assert jnp.array_equal(
        first.search_tree.executed_simulation_call_count,
