...
from scacchi.dirichlet_mctx.base import RecurrentFnOutput, RootFnOutput
from scacchi.dirichlet_mctx.outcomes import NO_OUTCOME, outcome_utility
from scacchi.dirichlet_mctx.policies import dirichlet_thompson_policy
...
def _policy(
    ...
    *,
    ...
    return dirichlet_thompson_policy(
        params=(),
        rng_key=rng_key,
        root=root,
...
def test_native_expand_fn_preserves_learned_alpha_and_tags_nonterminal():
    ...
    root = RootFnOutput(
        ...
        terminal_outcome=jnp.full((1,), int(NO_OUTCOME), dtype=jnp.int8),
...
def test_tree_stores_flat_messages_counts_and_values():
    ...
    assert {"edge_alpha", "edge_payload", "node_value_alpha"} <= set(
...
def test_structural_edge_counts_do_not_define_search_policy():
    ...
    node = NodeView(
        ...
        node_payload=jnp.array([65], dtype=jnp.int32),
        edge_alpha=action_alpha,
        edge_payload=edge_payload,
        edge_categorical_outcome=jnp.full((1, 2), int(NO_OUTCOME), dtype=jnp.int8),
    ...
    children = ChildrenView(
        ...
        categorical_outcome=jnp.full((1, 2), int(NO_OUTCOME), dtype=jnp.int8),
...
def test_default_max_depth_uses_complete_simulation_budget():
    ...
    # categorical descendants replace uncertain messages rather than adding
    # structural visit counts, and their distance propagates to the root.
    assert tree.summary().visit_counts[0, 0] == 0
    assert tree.summary().q_categorical_outcome[0, 0] == 1
    assert tree.summary().q_categorical_distance[0, 0] == terminal_depth
...
def test_custom_posterior_update_sees_children_and_runs_bottom_up():
    ...
    def custom_update(rng_key, context):
        ...
        child_has_message = context.children.node_payload[batch, action] > 0
        value_alpha = jnp.where(
            child_has_message[:, None],
            repaired_child_value,
            jnp.where(context.leaf.active[:, None], direct_value, node.value_alpha),
        )
    ...
    output = dirichlet_thompson_policy(
        params=(),
