...
def instantiate_tree_from_root(
    ...
    return Tree(
        ...
        node_categorical_outcome=jnp.full(
            ...
            int(NO_OUTCOME),
            dtype=jnp.int8,
        ).at[:, Tree.ROOT_INDEX].set(root.terminal_outcome.astype(jnp.int8)),
...
def expand(params: base.Params, rng_key: chex.PRNGKey, tree: Tree, recurrent_fn: base.RecurrentFn, simulation: Simulation, new_node_index: jax.Array) -> tuple[Tree, base.RecurrentFnOutput]:
...
def _gather_children(tree: Tree, node_index: jax.Array) -> ChildrenView:
    ...
    return ChildrenView(
        ...
        categorical_outcome=tree.node_categorical_outcome[
            batch_actions,
            safe_child,
        ],
...
def _set_node_update(
    ...
    count_delta = jnp.sum(
        jnp.where(legal, new_payload - old_payload, 0),
        axis=-1,
    )
    ...
    edge_alpha = jnp.where(
        unresolved[..., None],
        update.edge_alpha,
        tree.edge_alpha[batch, node_index],
    )
    return replace(
        tree,
        node_payload=_set_node(
            tree.node_payload,
            node_index,
            new_node_payload,
            active,
        ),
        edge_alpha=_set_node(
            tree.edge_alpha,
            node_index,
            edge_alpha,
            active,
        ),
        edge_payload=_set_node(
            tree.edge_payload,
            node_index,
            new_payload,
            active,
        ),
        node_value_alpha=_set_node(
...
def _categorize_node_and_publish(
    ...
    aligned_outcome = jnp.where(
        ...
        candidate_outcome,
        (num_outcomes - 1 - candidate_outcome).astype(jnp.int8),
    )
    ...
    return replace(
        ...
        edge_payload=_set_edge(
            tree.edge_payload,
            safe_parent,
            incoming_action,
            candidate_distance + jnp.asarray(1, dtype=jnp.int32),
            publish_edge,
        ),
    )
