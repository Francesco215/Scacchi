...
class TransformerNet(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.value_dir_out = nnx.Linear(value_hidden_dim, 3, rngs=rngs)
        self.value_conc_out = nnx.Linear(value_hidden_dim, 1, rngs=rngs)
        self.q_dir_linear = nnx.Linear(self.num_tokens * embed_dim, num_actions * 3, rngs=rngs)
        self.q_conc_linear = nnx.Linear(self.num_tokens * embed_dim, num_actions, rngs=rngs)
    ...
    def __call__(self, x: jax.Array, *, train: bool) -> tuple[jax.Array, jax.Array, jax.Array]:
        ...
        v = jax.nn.relu(self.value_hidden(flat))
        r_V = self.value_dir_out(v)
        c_V = self.value_conc_out(v).reshape((-1,))
        alpha_V = 1.0 + jax.nn.softplus(c_V)[..., None] * jax.nn.softmax(r_V, axis=-1)

        r_Q = self.q_dir_linear(flat).reshape((-1, self.num_actions, 3))
        c_Q = self.q_conc_linear(flat)
